...
通过 kubebuilder 命令添加一个 V2 版本的 API
```shell
# 添加api /apis/apps.clusterops.io/v2/namespaces/<namespace>/applications/<name>
➜ kubebuilder create api --group apps --version v2 --kind Application
kubebuilder create api --group apps --version v2 --kind Application
Create Resource [y/n]          # 创建资源
y
Create Controller [y/n]        # Controller 已经创建过了
n
Writing kustomize manifests for you to edit...
Writing scaffold for you to edit...
api/v2/application_types.go
api/v2/groupversion_info.go
Update dependencies:
$ go mod tidy
Running make:
$ make generate
test -s /Users/aoyang/workspace/MyOperatorProjects/clusterops-operator/bin/controller-gen && /Users/aoyang/workspace/MyOperatorProjects/clusterops-operator/bin/controller-gen --version | grep -q v0.12.0 || \
	GOBIN=/Users/aoyang/workspace/MyOperatorProjects/clusterops-operator/bin go install sigs.k8s.io/controller-tools/cmd/controller-gen@v0.12.0
/Users/aoyang/workspace/MyOperatorProjects/clusterops-operator/bin/controller-gen object:headerFile="hack/boilerplate.go.txt" paths="./..."
Next: implement your new API and generate the manifests (e.g. CRDs,CRs) with:
$ make manifests
```
添加一个 V2 版本的 API 后，可以看到 api 目录下多了一个 v2 目录，另外 main.go 和 PROJECT 发生了变化，新增的内容和 V1 版本是对称一致的。

下面需要在 `api/v2/application_types.go` 文件中实现和 V1 版本完全一样的代码逻辑（除了 package v2 这一行有差异）。但是有了多个版本的 API，在 API Server 中却只能指定持久化一个版本，这里选择持久化 V1 版本，所以需要在 V1 版本的 Application 上增加一行注解 `//+kubebuilder:storageversion`。

这时还应该添加一个 Webhook 用来接收 API Server 的 conversion 回调请求，[参考文档](https://book.kubebuilder.io/multiversion-tutorial/webhooks.html)。

### 3. API分组支持

有时会在一个 Operator 项目中实现多个控制器来管理不同的 API 资源组。 
...
domain: clusterops.io
layout:
- go.kubebuilder.io/v4
multigroup: true         # 会在 PROJECT 文件中增加一行 multigroup:true，这行配置会告诉 Kubebuilder 后面新增的API不要放到 api/<version> 目录下，而是放到 apis/<group>/<version> 目录下；另外控制器的代码也不再放到 controllers 目录下，而是放到 controllers/<group> 目录下
projectName: clusterops-operator
repo: github.com/ahwhy/clusterops-operator
resources:
- api:
    crdVersion: v1
    namespaced: true
  ...
  group: apps
  kind: Application
  path: github.com/ahwhy/clusterops-operator/api/v1
  version: v1
  webhooks:
    defaulting: true
    validation: true
...
- api:
  ...
  group: apps
  kind: Application
  path: github.com/ahwhy/clusterops-operator/api/v2
  version: v2
version: "3"
```
接着还有一些手动工作需要完成，需要手动挪动 api、controllers 目录内相关源文件，并更新 源文件内的 import 部分的引用
